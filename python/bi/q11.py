...
Count triangles of persons located in ``country`` where all three knows edges
...
persons (reading each knows edge's ``kd`` creation-day property), then count
triangles a<b<c with all three edges present.
"""
...
def q11_friend_triangles(g, country_name: str, start_day: int, end_day: int) -> int:
    ...
    adj = {}  # person -> set of in-country knows neighbors with edge kd in window
    for a in in_country:
        nbrs = set()
