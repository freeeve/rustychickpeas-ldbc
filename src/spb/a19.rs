...
pub fn run(
    ...
    // Render the display name once (label, else the bare uri `?topic` binds to).
    let mut rows: Vec<(String, usize, i64, String)> = acc
        .into_iter()
        .map(|(topic, (cnt, ms, date))| {
            let name = pstr(g, topic, "label")
                .or_else(|| pstr(g, topic, "uri"))
                .unwrap_or("?")
                .to_string();
            (name, cnt, ms, date.to_string())
        })
        .collect();
    // ORDER BY DESC(?maxDate), DESC(?topicsCount), then name asc for determinism.
    rows.sort_by(|a, b| {
        b.2.cmp(&a.2).then_with(|| b.1.cmp(&a.1)).then_with(|| a.0.cmp(&b.0))
    });
    rows.truncate(limit);
    rows.into_iter().map(|(name, cnt, _ms, date)| (name, cnt, date)).collect()
}
