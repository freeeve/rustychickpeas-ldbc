...
//! `CreativeWork` label our `full_text_search` indexes on; and `cwork:tag` is the
...
pub fn run(g: &GraphSnapshot, word: &str, limit: usize) -> Vec<(String, String)> {
    ...
    for w in g.full_text_search("CreativeWork", "title", word).iter() {
        ...
        if g.prop_str(w, "title").is_none() {
            continue;
        }
        if g.neighbors_by_type(w, Direction::Outgoing, "category")
            .next()
            .is_none()
        {
            continue;
...
"#;
    ...
    fn policy_titled_work_emits_a_row_per_tag() {
        ...
        assert_eq!(
            ...
            vec![
                (
                    "http://ex/cw-policy".to_string(),
                    "http://dbpedia.org/resource/Data_protection".to_string()
                ),
                (
                    "http://ex/cw-policy".to_string(),
                    "http://sws.geonames.org/london".to_string()
                ),
            ]
        );
    }
