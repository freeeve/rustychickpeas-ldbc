# 042 — SPB advanced q19

**Goal.** Faithful hand-coded rustychickpeas implementation of SPB advanced q19 (no SPARQL engine), matching the official query template.

**Source SPARQL:** `data/spb/ldbc_spb_bm_2.0/datasets_and_queries/sparql/advanced/aggregation_standard/query19.txt` (local SPB checkout; the `{{{...}}}` placeholders are substitution parameters).

**SPB query spec (verbatim header):**
```
 Query name : query19
 Query Description : 
 Retrieve most popular topics that creative works are tagged with in a fixed date-time range, ordered by most-recent modification date.
 Additional constraint is the type of creative tagging those topics and its audience.
```
