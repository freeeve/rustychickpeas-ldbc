# 045 — SPB advanced q22

**Goal.** Faithful hand-coded rustychickpeas implementation of SPB advanced q22 (no SPARQL engine), matching the official query template.

**Source SPARQL:** `data/spb/ldbc_spb_bm_2.0/datasets_and_queries/sparql/advanced/aggregation_standard/query22.txt` (local SPB checkout; the `{{{...}}}` placeholders are substitution parameters).
...
```
 ...
 A faceted search query, several iterations of that query follow, adding up more constraints to previous iterations:
   ...
   - Finally selecting title and date of creation by adding a constraint on: month of the year
