...
## Result
7.8ms -> 6.5ms (~1.2x). Loader now labels untyped resources `Facet`, so node_by_uri resolves the primaryFormat/webDocumentType targets via the cached uri index; the filters compare node ids instead of reading each edge target's uri. Modest because a14's cost is the per-work star scan over all ~38k CreativeWorks, not the facet lookup. Parity 30/30.
