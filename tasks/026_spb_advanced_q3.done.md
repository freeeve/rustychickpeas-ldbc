...
**Goal.** Faithful hand-coded rustychickpeas implementation of SPB advanced q3 (no SPARQL engine), matching the official query template.

**Source SPARQL:** `data/spb/ldbc_spb_bm_2.0/datasets_and_queries/sparql/advanced/aggregation_standard/query3.txt` (local SPB checkout; the `{{{...}}}` placeholders are substitution parameters).

**SPB query spec (verbatim header):**
```
 Query name : query3
 Query Description : 
 Retrieve creative works that have been modified in a time range of one hour
 reasoning : owl:DataProperty, rdfs:isDefinedBy
```
