# 039 — SPB advanced q16

**Goal.** Faithful hand-coded rustychickpeas implementation of SPB advanced q16 (no SPARQL engine), matching the official query template.

**Source SPARQL:** `data/spb/ldbc_spb_bm_2.0/datasets_and_queries/sparql/advanced/aggregation_standard/query16.txt` (local SPB checkout; the `{{{...}}}` placeholders are substitution parameters).

**SPB query spec (verbatim header):**
```
 Query name : query16
 Query Description : 
 Retrieve a list of N creative works, their tags, categories and titles that contain 
 the term "policy", ordered by their tag
reasoning rdfs:subClassOf
