...
- **Graphalytics LCC** `lcc` / `lcc_count_range` (src/graphalytics/mod.rs:261-403) — hand-rolls a
  ...
  (mod.rs:334-397, 403) to count edges among N(v). The Graphalytics agent was firm: LCC wants the
...
- **BI Q11** `q11_friend_triangles` (src/bi/faithful_a.rs; python/bi/q11.py) — counts triangles a<b<c
  in a DATE-FILTERED knows subgraph. Note: Q11 filters edges by `kd` window, so the primitive may need
  an optional edge predicate, or Q11 stays partly bespoke. LCC is unfiltered.

## Prior art / naming

Triangle counting; local clustering coefficient; neighbor (set) intersection. Candidates:
`triangles` / `triangle_count` (per node), `count_neighbor_edges`, or a lower-level
...
- Edge filter (Q11's kd window) is the divergence; LCC has none. Possibly two consumers only if the
