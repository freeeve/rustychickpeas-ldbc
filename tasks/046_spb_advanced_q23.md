# 046 — SPB advanced q23

**Goal.** Faithful hand-coded rustychickpeas implementation of SPB advanced q23 (no SPARQL engine), matching the official query template.

**Source SPARQL:** `data/spb/ldbc_spb_bm_2.0/datasets_and_queries/sparql/advanced/aggregation_standard/query23.txt` (local SPB checkout; the `{{{...}}}` placeholders are substitution parameters).
...
```
 ...
 A faceted search query, several iterations of that query follow, adding up more constraints to previous iterations:
   ...
   - Finally returning days and count of tags grouped by tag
