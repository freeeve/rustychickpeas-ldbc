# 036 — SPB advanced q13

**Goal.** Faithful hand-coded rustychickpeas implementation of SPB advanced q13 (no SPARQL engine), matching the official query template.

**Source SPARQL:** `data/spb/ldbc_spb_bm_2.0/datasets_and_queries/sparql/advanced/aggregation_standard/query13.txt` (local SPB checkout; the `{{{...}}}` placeholders are substitution parameters).

**SPB query spec (verbatim header):**
```
 Query name : query13
 Query Description : 
 Retrieve a list of N creative works, the things they are tagged about and mention, their modification date. Limit results by two categories.
 (star query)
