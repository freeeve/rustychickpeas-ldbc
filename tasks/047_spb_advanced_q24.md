# 047 — SPB advanced q24

**Goal.** Faithful hand-coded rustychickpeas implementation of SPB advanced q24 (no SPARQL engine), matching the official query template.

**Source SPARQL:** `data/spb/ldbc_spb_bm_2.0/datasets_and_queries/sparql/advanced/aggregation_standard/query24.txt` (local SPB checkout; the `{{{...}}}` placeholders are substitution parameters).

**SPB query spec (verbatim header):**
```
 Query name : query24
 Query Description : 
 Retrieve a time-line of relatedness between two entities in dataset. Query explores correlations of entities tagged
 together in creative works i.e. a history of interactions.
 Additional constraint of time interval of 6 months limits the result.
```
