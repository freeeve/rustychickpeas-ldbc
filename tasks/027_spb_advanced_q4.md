...
**Goal.** Faithful hand-coded rustychickpeas implementation of SPB advanced q4 (no SPARQL engine), matching the official query template.

**Source SPARQL:** `data/spb/ldbc_spb_bm_2.0/datasets_and_queries/sparql/advanced/aggregation_standard/query4.txt` (local SPB checkout; the `{{{...}}}` placeholders are substitution parameters).

**SPB query spec (verbatim header):**
```
 Query name : query4
 Query Description : 
 Retrieve the greatest amount of types of creative works that have been modified in some larger reriod of time (2/3 of a whole year).
 reasoning : rdf:type
```
