...
**Goal.** Faithful hand-coded rustychickpeas implementation of SPB advanced q6 (no SPARQL engine), matching the official query template.

**Source SPARQL:** `data/spb/ldbc_spb_bm_2.0/datasets_and_queries/sparql/advanced/aggregation_standard/query6.txt` (local SPB checkout; the `{{{...}}}` placeholders are substitution parameters).

**SPB query spec (verbatim header):**
```
 Query name : query6
 Query Description : 
 Retrieve the N most popular topic types that creative works are about. Adds a filter on coverage and audience properties.
 reasoning : owl:ObjectProperty, rdf:type
```
